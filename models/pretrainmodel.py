...
class SAINT_encoding(nn.Module):
    def __init__(
        self,
        *,
        categories, # tuple with number of categories for each categorical feature
        num_continuous, # number of continuous features
        dim, # dimension of the embeddings
        depth, # depth of the transformer (number of attention layers)
        heads, # number of heads
        dim_head = 16, # CHECK
        dim_out = 1, # CHECK
        mlp_hidden_mults = (4, 2), # CHECK
        mlp_act = None, # CHECK
        num_special_tokens = 0, # CHECK
        attn_dropout = 0.,
        ff_dropout = 0., # CHECK
        cont_embeddings = 'MLP', # embedding type for continuou variables (can be 'MLP','Noemb' or 'pos_singleMLP')
        attentiontype = 'col', # specify which attention block we want to include in the network
        ):
        super().__init__()
        assert all(map(lambda n: n > 0, categories)), 'number of each category must be positive'
        ...
        # for automatically offsetting unique category ids to the correct position in the categories embedding table
        # during the creation of embeddings for categoriacl features the categories of all different features are unified into a single tensor, thus we need to kow the offsets
        categories_offset = F.pad(torch.tensor(list(categories)), (1, 0), value = num_special_tokens)
        categories_offset = categories_offset.cumsum(dim = -1)[:-1]
